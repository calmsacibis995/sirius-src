...
/* Name        - s390x_elf.c   					    */
/* 								    */
/* Function    - s390x machine dependent and ELF file class 	    */
...
typedef struct __plt0 {
	char	st[6];		/* stg	%r1,56(%r15)		    */
	char	larl[2];	/* larl	%r1,			    */
	Sword	got;		/* __GLOBAL_OFFSET_TABLE__	    */
	char	mvc[6];		/* mvc	48(4,%r15),8(%r1)	    */
	char	loff[6];	/* lg	%r1,8(%r1)		    */
	char	br[2];		/* br	%r1			    */
	short	nop[3];		/* Padding			    */
...
typedef struct __plt1 {
	char	larl[2];	/* larl	%r1,			    */
	Sword	gotent;		/* ep@GOTENT              	    */
	char	lgot[6];	/* lg	%r1,0(%r1)		    */
	char	br[2];		/* br	%r1			    */
	char	basr[2];	/* basr	%r1,0			    */
	char	loff[6];	/* lgf	%r1,12(%r1)		    */
	char	jg[2];		/* jg				    */
	Sword	plt0;		/* offset(plt0)			    */
	Sword	gotoff;		/* offset(symbol table)		    */
} __attribute__ ((packed)) plt1_t;

typedef struct __dynplt {
	char	lgr[4];		/* lgr	%r1,%r15		    */
	char	ahi[4];		/* aghi	%r15,-SA(MINFRAME+...)      */
	char	stysp[6];	/* stg	%r1,-16(%r15)		    */
	char	larl[2];	/* larl	%r1,			    */
	uchar_t dyndata[4];	/* offset(dyndata)		    */
	char	stydd[6];	/* stg	%r1,-8(%r15)		    */
	char	jg[2];		/* jg				    */
 ...
 *
 *	lgr	%r1,%r15
 *	aghi	%r15,-SA(MINFRAME + 2*CLONGSIZE) 
 *	stg	%r1,-8(%r15)
 *	larl	%r1,dyndata
 *	stg	%r1,-8(%r15)
 *	jg	elf_plt_trace
    ...
    uint_t symndx, uint_t pltndx, caddr_t to, uint_t sb_flags, int *fail)
{
	...
	if (*dyn_plt == 0) {
		...
		*dyndata++ = (uint_t)sb_flags;
		symp = (Sym *)dyndata;
		*symp = *sym;
		symp->st_name += (Xword)STRTAB(dlmp);
...
#if 0
{
	...
	while (relbgn < relend) {
		...
		default:
			 ...
			 */
			if (do_reloc_rtld(rtype, (uchar_t *)roffset, (Xword *)&value,
