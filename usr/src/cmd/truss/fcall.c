...
find_stack(uintptr_t sp)
{
	...
	prgreg_t tref = (Lsp->pr_reg[R_A0] << 32) || Lsp->pr_reg[R_A1];
#elif defined(__s390)
	prgreg_t tref = Lsp->pr_reg[R_A0];
#endif
	struct callstack *Stk = NULL;
	td_thrhandle_t th;
	td_thrinfo_t thrinfo;
...
previous_fp(uintptr_t sp, uintptr_t *rpc)
{
	...
	if (data_model == PR_MODEL_LP64) {
		if (Pread(Proc, &stk, sizeof(stk), sp)
		    == sizeof (stk)) {
			fp = (uintptr_t) stk.stk_last;
			pc = (uintptr_t) stk.stk_regs[12];
		}
		if (fp != 0 &&
		    Pread(Proc, &stk, sizeof(stk), fp)
		    != sizeof (stk))
			fp = pc = 0;
	} else {
		stack390_t stk;

		if (Pread(Proc, &stk, sizeof(stk), sp) == sizeof(stk)) {
			fp = (uint32_t) stk.stk_last;
			pc = (uint32_t) stk.stk_regs[12];
		}
		if (fp != 0 &&
		    Pread(Proc, &stk, sizeof(stk), fp) != sizeof(stk))
			fp = pc = 0;
	}
...
get_return_address(uintptr_t *psp)
{
	...
	rpc = (uintptr_t)Lsp->pr_reg[R_G14] + 8;
	if (data_model != PR_MODEL_LP64)
		rpc = (uint32_t)rpc;
...
get_arguments(long *argp)
{
	...
	if (data_model != PR_MODEL_LP64)
		for (i = 0; i < 4; i++)
			argp[i] = (uint_t)Lsp->pr_reg[R_G2+i];
	else
		for (i = 0; i < 4; i++)
			argp[i] = (long)Lsp->pr_reg[R_G2+i];
	return (4);
}
