...
#if defined(__s390x)
#define	IF_s390x(true, false)	true
#else
#define	IF_s390x(true, false)	false
#endif

#if defined(__s390)
#define	IF_s390(true, false)	true
#else
#define	IF_s390(true, false)	false
#endif
...
# ifdef __s390x
/*
 * The System z ABI requires that int parameters are automatically sign
 * extended. For positive ints this is not a problem but for APIs such as 
 * lseek the off_t parameter may be negative and must be sign extended.
 * For these calls we provide some "thunking" code to allow this to happen.
 */
#  define lseek32	_lseek32
#  define pread32	_pread32
#  define pwrite32	_pwrite32
   extern off_t		lseek32();
   int	_cpcmd32();
# endif
