...
/*
 * The System z platform has special requirements with respect to the
 * recording of page modification - it is based on the physical page and
 * may be set by a read I/O operation. 
 */
#ifndef __s390x
# define VOPCHK_PAGE(vn, off, len, pl)
# define VOPSET_PAGE(vn, off, len)
#else
static __inline__ void vopchk_page(vnode_t *, offset_t, size_t, page_t **);
static __inline__ void vopset_page(vnode_t *, offset_t, size_t);

# define VOPCHK_PAGE(vn, off, len, pl)					\
	if (err == 0)							\
		vopchk_page(vn, off, len, pl)

# define VOPSET_PAGE(vn, off, len) vopset_page(vn, off, len)
...
static void
vopchk_page(vnode_t *vp, offset_t off, size_t len, page_t **pl)
{									
	page_t	 *pp;							
	long	 key = 0;						
	int	 i_pl = 0;						
	...
	if (pl == NULL) {						
		while (len > 0) {
			...
			if ((pp != NULL) && !(pp->p_state & P_INITIO)) {		
				frame = pp->p_pagenum << MMU_PAGESHIFT;
				__asm__ ("	sske	%0,%1\n"
					 : : "r" (key), "r" (frame));
				PP_SETINIT(pp);
				hat_clrrefmod(pp);
		...
		for (pp = pl[i_pl]; pp != NULL; pp = i_pl++) {		
			if (!PP_ISINIT(pp)) {
				frame = pp->p_pagenum << MMU_PAGESHIFT;
				__asm__ ("	sske	%0,%1\n"
					 : : "r" (key), "r" (frame));
...
/* Name		- vopset_page.                                      */
/*                                                                  */
/* Function	- Set p_state to indicate I/O has been performed to */
/*                this page so that fsflush() will do its thing.    */
/*                                                                  */
/*------------------------------------------------------------------*/

static __inline__ void
vopset_page(vnode_t *vp, offset_t off, size_t len)
{
	page_t	*pp;							
	long	key = 0;						
	int	i_pl = 0;						
