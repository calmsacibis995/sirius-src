...
#else
	uchar_t		p_embed;	/* x86/s390x - changes p_mapping & p_index */
#endif
...
#define	P_INITIO	0x04		/* initial i/o performed to page (s390x) */
...
/*
 * Because System z associates its page modified state with a physical
 * page, read I/O will cause the page to be seen as dirty. To get things
 * to behave correctly we will reset the mod/ref bits when the first read
 * is done to a page and then indicate that the page has had an initial I/O 
 * performed so that fsflush() etc. will correctly identify candidates for 
 * flushing.
 */
#ifndef __s390x
...
# define PP_CLRINIT(pp)		((pp)->p_state &= ~P_INITIO)
#endif
