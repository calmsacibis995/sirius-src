...
/* Name        - kobj_reloc.c.					    */
/* 								    */
/* Function    - Perform relocations for kernel modules.            */
/* 								    */
/* Name	       - Neale Ferguson					    */
/* 								    */
/* Date        - September, 2007 				    */
...
/*------------------------------------------------------------------*/
/*                                                                  */
/* Name		- tnf_reloc_resolve.                                */
/*                                                                  */
/* Function	- The kernel run-time linker calls this to try to   */
/*		  resolve a reference it can't otherwise resolve.   */
/*		  We see if it's marking a probe control block or a */
/*		  probe tag block; if so, we do the resolution and  */
/*		  return 0. If not, we return 1 to show that we     */
...
/*------------------------------------------------------------------*/
/*                                                                  */
/* Name		- sdt_reloc_resolve.                                */
/*                                                                  */
/* Function	- The "statically defined tracing" (SDT) provider   */
/*		  for DTrace uses a mechanism similar to TNF, but   */
/*		  somewhat simpler.  (Surprise, surprise.)  The SDT */
/*		  mechanism works by replacing calls to the 	    */
/*		  undefined routine __dtrace_probe_[name] with nop  */
/*		  instructions. The relocations are logged, and SDT */
/*		  itself will later patch the running binary 	    */
/*		  appropriately.				    */
/*		                               		 	    */
/*------------------------------------------------------------------*/
...
#if 0
/* S390X FIXME - What needs to be checked for with s390x? */
	/*
...
#endif
	/* We just patch all dtrace stuff to NOP at the moment */
	instr  = ((void *) instr) - 2;
	*instr = SDT_NOP;
...
/* Name		- do_relocate.                                      */
/*                                                                  */
/* Function	- Manage the relocation process.                    */
/*		                               		 	    */
/*------------------------------------------------------------------*/
	...
	Addr baseaddr)
{
	...
	while (reladdr < rend) {
		...
		if (stndx >= mp->nsyms) {
			_kobj_printf(ops,
			    "do_relocate: bad stndx %d (max: %d)\n"
			    " at 0x%llx file=%s\n", 
			    stndx, mp->nsyms, off, mp->filename);
			return (-1);
		}
		if ((rtype > R_390_NUM) || IS_TLS_INS(rtype)) {
		...
		/*
		 * if R_390_RELATIVE, simply add base addr
		 * to reloc location
		 */
...
/* Name		- do_relocations.                                   */
/*                                                                  */
/* Function	- Perform the relocation.                           */
/*		                               		 	    */
/*------------------------------------------------------------------*/
