...
extern hrtime_t tick2ns(hrtime_t);
extern void 	unlock_hres_lock(uint32_t *);

extern uint64_t nano2tod(hrtime_t);
extern uint64_t ticks2tod(hrtime_t);
extern hrtime_t tod2ticks(void);
extern hrtime_t tod2nano(void);
...
#define NSECS_PER_TICK	10000000	/* 10 milliseconds per tick  */
#define TOD2NANO_SCALE  8192		/* Value to obtained nano from TOD */
#define TOD_EPOCH	0x7d91048bca000000	/* 1970-01-01 00:00:00 */
//#define TOD_EPOCH	0x7d9104a0c5180000	/* 1970-01-01 00:00:00 */
#define CVT2PICO	244140		/* TOD unit to picoseconds factor */
#define TODFACTOR	1000000		/* TOD scaling factor */
#define US_TO_NS	1000		/* Convert from micros to nanos */
#define US_MASK		0xfff		/* Mask out all but fractional micros */
#define US_SHIFT	12		/* Remove fractional micros */
...
static hrtime_t __inline__
cvt2nano(uint64_t tod)
{
	return (((tod >> US_SHIFT) * US_TO_NS) + (((tod & US_MASK) * CVT2PICO) / TODFACTOR));
}
