...
#include <sys/asm_linkage.h>
	...
	ENTRY(strlcpy)
	stmg	%r6,%r15,24(%r15)
	lgr	%r6,%r3			// Copy source
	lgr	%r8,%r2			// Copy destination
	lgr	%r5,%r4			// Copy max length
	larl	%r1,eosTab		// Point at end of string table
0:
	trt	0(256,%r6),0(%r1)	// Locate end of string
	jnz	1f			// Go if found

	aghi	%r6,256			// Next block
	j	0b			// Try again
	
1:
	sgr	%r1,%r3			// Determine source length
	lgr	%r2,%r1			// Copy
	ltgr	%r4,%r4			// Do we want 0 bytes?
	jz	6f			// Yes... Get out of here

	ltgr	%r2,%r2			// Do we have 0 bytes?
...
2:
	lgr	%r10,%r3		// Get *str2
	lgr	%r1,%r8			// Save *str1
	cgr	%r2,%r4			// Is length of src < wanted?
	jnl	3f			// No... Skip

	lgr	%r4,%r2			// Use max length
3:
	lgr	%r9,%r4			// Get l'str1
	lgr	%r11,%r4		// Get l'str1
4:	mvcle	%r8,%r10,0		// Move string
	jo	4b			// If not complete.. keep checking	

	cgr	%r4,%r5			// Is length moved < source
	jl	5f			// Yes... Skip

	aghi	%r4,-1			// Backpeddle 
5:
	agr	%r4,%r1			// Point at end of string
