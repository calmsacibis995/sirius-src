...
#define	U_REG		((uint64_t)(&UP->u_reg[0]))
#define	U_AR0		((uint64_t)(&UP->u_ar0))
#define	U_PSARGS	((uint64_t)(&UP->u_psargs[0]))
#define	U_SIGNAL	((uint64_t)(&UP->u_signal[0]))
#define	U_CODE		((uint64_t)(&UP->u_code))
#define	U_ADDR		((uint64_t)(&UP->u_addr))
#define	U_END		((uint64_t)sizeof (user_t))
...
again:
	...
	case 9:		/* PTRACE_SINGLESTEP */
		...
		if (cp->flags & CS_SETREGS) {
			...
			ps->pr_lwp.pr_reg[R_G0]   = cp->user.u_reg[REG_G0];
			...
			ps->pr_lwp.pr_reg[R_G8]   = cp->user.u_reg[REG_G8];
			ps->pr_lwp.pr_reg[R_G9]   = cp->user.u_reg[REG_G9];
			ps->pr_lwp.pr_reg[R_G10]  = cp->user.u_reg[REG_G10];
			ps->pr_lwp.pr_reg[R_G11]  = cp->user.u_reg[REG_G11];
			ps->pr_lwp.pr_reg[R_G12]  = cp->user.u_reg[REG_G12];
			ps->pr_lwp.pr_reg[R_G13]  = cp->user.u_reg[REG_G13];
			ps->pr_lwp.pr_reg[R_G14]  = cp->user.u_reg[REG_G14];
			ps->pr_lwp.pr_reg[R_G15]  = cp->user.u_reg[REG_G15];
			cmd = PCSREG;
			iov[0].iov_base = (caddr_t)&cmd;
			iov[0].iov_len = sizeof (long);
			iov[1].iov_base = (caddr_t)&ps->pr_lwp.pr_reg[R_G2];
			iov[1].iov_len = sizeof (ps->pr_lwp.pr_reg);
			if (writev(cp->ctlfd, iov, 2) < 0)
				goto tryagain;
		}
		if (addr != 1 &&	/* new virtual address */
		    (addr & ~01) != cp->user.u_psw.pc) {
			runctl[0] = PCSVADDR;
			runctl[1] = (addr & ~01);
...
ProcUpdate(cstatus_t *cp)
{
	...
	if (cp->flags & CS_SETREGS) {
		...
		iov[0].iov_base = (caddr_t)&cmd;
		iov[0].iov_len = sizeof (long);
		iov[1].iov_base = (caddr_t)&ps->pr_lwp.pr_reg[4];
		iov[1].iov_len = sizeof (ps->pr_lwp.pr_reg);
...
MakeUser(cstatus_t *cp)
{
	...
	cp->user.u_reg[REG_G0]   = ps->pr_lwp.pr_reg[R_G0];
	...
	cp->user.u_reg[REG_G8]   = ps->pr_lwp.pr_reg[R_G8];
	cp->user.u_reg[REG_G9]   = ps->pr_lwp.pr_reg[R_G9];
	cp->user.u_reg[REG_G10]  = ps->pr_lwp.pr_reg[R_G10];
	cp->user.u_reg[REG_G11]  = ps->pr_lwp.pr_reg[R_G11];
	cp->user.u_reg[REG_G12]  = ps->pr_lwp.pr_reg[R_G12];
	cp->user.u_reg[REG_G13]  = ps->pr_lwp.pr_reg[R_G13];
	cp->user.u_reg[REG_G14]  = ps->pr_lwp.pr_reg[R_G14];
	cp->user.u_reg[REG_G15]  = ps->pr_lwp.pr_reg[R_G15];
	cp->user.u_ar0 = (greg_t *)REGADDR;
	cp->user.u_code = ps->pr_lwp.pr_info.si_code;
	cp->user.u_addr = ps->pr_lwp.pr_info.si_addr;
	memcpy(&cp->user.u_psw, &ps->pr_lwp.pr_reg[R_PSWM], sizeof(pswg_t));
