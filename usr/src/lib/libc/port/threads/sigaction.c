...
setcontext(const ucontext_t *ucp)
{
	...
	if (setcontext_enforcement) {
		...
		uc.uc_mcontext.aregs[0] = (uint32_t) self;
#elif defined(__s390x)
		uc.uc_mcontext.aregs[0] = (uint32_t) ((uint64_t) self >> 32);
		uc.uc_mcontext.aregs[1] = (uint32_t) ((uint64_t) self & 0xffffffff);
#else
#error "none of __sparc, __amd64, __i386, __s390, __s390x defined"
#endif
	}
