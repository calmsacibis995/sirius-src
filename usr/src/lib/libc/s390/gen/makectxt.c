...
_makecontext(ucontext_t *ucp, void (*func)(), int argc, ...)
{
	...
	reg = (greg_t *) &ucp->uc_mcontext.gregs;
	psw = &ucp->uc_mcontext.psw;
	pc  = (uint32_t) func;
	psw->pc = (uint64_t) pc;
	...
	size += (argc > 5 ? argc - 5 : 0) * sizeof (long);
	...
	for (argno = 0; argno < argc; argno++) {
		if (argno < 5)
			*tsp++ = reg[REG_G2 + argno] = va_arg(ap, long);
		else
			*tsp++ = va_arg(ap, long);
	}
	...
	reg[REG_SP]  = (greg_t)sp;
	reg[REG_G14] = (greg_t)resumecontext;	/* return pc */
}
...
__makecontext_v2(ucontext_t *ucp, void (*func)(), int argc, ...)
{
	...
	reg[REG_SP]  = (greg_t)sp;
	reg[REG_G14] = (greg_t)resumecontext;
}
